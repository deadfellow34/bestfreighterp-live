/**
 * ESKİ sqlite3 AĞIR PERFORMANS TESTİ
 * Bu test eski sqlite3 paketi ile çalışır
 * Kullanım: node perf_test_sqlite3.js
 * 
 * NOT: Bu testi çalıştırmak için önce: npm install sqlite3
 */
...
console.log('╔═══════════════════════════════════════════════════════════╗');
console.log('║     ESKİ sqlite3 AĞIR PERFORMANS TESTİ                    ║');
console.log('╚═══════════════════════════════════════════════════════════╝\n');
...
async function runTests() {
  ...
  for (let i = 0; i < 1000; i++) {
    await dbGet('SELECT * FROM loads WHERE id = ?', [(i % 87) + 1]);
  }
  ...
  for (let i = 0; i < 1000; i++) {
    const res = await dbRun('INSERT INTO logs (username, action, entity, field, old_value, new_value) VALUES (?, ?, ?, ?, ?, ?)', 
      ['STRESS_TEST', 'benchmark-' + i, 'performance', 'value', 'old_' + i, 'new_' + i]);
    await dbRun('UPDATE logs SET action = ? WHERE id = ?', ['updated-' + i, res.lastID]);
    await dbRun('DELETE FROM logs WHERE id = ?', [res.lastID]);
  }
  await dbExec('COMMIT');
  ...
  // ═══════════════════════════════════════════════════════════
  // TEST 7: 2000x SELECT (prepared statement yok - her seferinde parse)
  // ═══════════════════════════════════════════════════════════
  console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('TEST 7: 2000x SELECT (no prepared stmt cache)');
  const start7 = Date.now();
  for (let i = 0; i < 2000; i++) {
    await dbGet('SELECT id, position_no, customer_name, truck_plate FROM loads WHERE id = ?', [(i % 87) + 1]);
  }
  ...
  for (let i = 0; i < 50; i++) {
    await dbGet('SELECT COUNT(*) as c FROM loads');
    await dbGet('SELECT COUNT(*) as c FROM documents');
    await dbGet('SELECT COUNT(*) as c FROM logs');
    await dbGet('SELECT COUNT(*) as c FROM position_expenses');
    await dbGet('SELECT COUNT(*) as c FROM users');
    await dbGet('SELECT COUNT(*) as c FROM trucks');
    await dbGet('SELECT COUNT(*) as c FROM trailers');
    await dbGet('SELECT COUNT(*) as c FROM seals');
    await dbGet('SELECT COUNT(*) as c FROM companies');
    await dbGet('SELECT COUNT(*) as c FROM vizebest_entries');
  }
  ...
  // ═══════════════════════════════════════════════════════════
  // TEST 9: 5000x INSERT batch (transaction içinde)
  // ═══════════════════════════════════════════════════════════
  console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  ...
  for (let i = 0; i < 5000; i++) {
    const res = await dbRun('INSERT INTO logs (username, action, entity, field) VALUES (?, ?, ?, ?)',
      ['BATCH_TEST', 'insert-' + i, 'stress', 'field-' + i]);
  ...
  for (let i = 0; i < 500; i++) {
    // Read
    await dbGet('SELECT * FROM loads WHERE id = ?', [(i % 87) + 1]);
    await dbAll('SELECT id, position_no FROM loads LIMIT 10');
    
    // Write (log)
    const res = await dbRun('INSERT INTO logs (username, action, entity) VALUES (?, ?, ?)', 
      ['MIXED_TEST', 'action-' + i, 'test']);
    
    // Read again
    await dbGet('SELECT COUNT(*) as c FROM logs WHERE id > ?', [res.lastID - 100]);
  ...
  // ═══════════════════════════════════════════════════════════
  console.log('\n╔═══════════════════════════════════════════════════════════╗');
  console.log('║                    SONUÇ ÖZETİ (sqlite3)                  ║');
  console.log('╠═══════════════════════════════════════════════════════════╣');
  ...
  console.log('║  Test 7  (2000x SELECT):        ' + formatMs(results.test7).padEnd(24) + '║');
  ...
  console.log('║                                                           ║');
  console.log('║  better-sqlite3 ile bu testler 5-10x daha HIZLI olur!     ║');
  console.log('╚═══════════════════════════════════════════════════════════╝');
